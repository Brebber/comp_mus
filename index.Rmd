...
For both groups I tried to pick bands that sound as distinct as possible from each other, however this list is definitely too limited to provide a complete overview of both genres. The non-Metal bands I chose are varied enough but still don't provide an overview of the genre in its entirety and 3 of the 5 metal bands I chose fall into different categories of the Death Metal subgenre.\ As 'non-Metal' is even more ambiguous I refer to the songs in that playlist as Rock songs in this portfolio, keep in mind that means 'Rock-but-not-Metal'.
...
Led Zeppelin - 'Immigrant Song'; Considered Heavy Metal at the time, a little closer to Rock than Metal in my opinion.\
...
I will be exploring this topic more in depth in [this tab](#emotion).
...
```
*** 
Metal tracks tend to have more varying tempos than Rock tracks. Which could be explained by the fact that the Metal songs I picked tend to feature more tempo changes than the Rock songs I picked. The mean tempo of the songs in both playlists does not seem to be a significant factor in which the playlists differ.
...
```
***
The key a song is written in plays a major role in the emotional perception of a track. Minor keys are often deemed sadder or less energetic, while major keys are often described as happier or more energetic. As seen in the valence and energy plot on the [corpus tab](#corpus), these descriptions aren't always accurate.\
...
### Analysis of a Classic (or Cliche)
```{r stwy}
stairway <-
...
As this song is in the key of Am, the keygram seems to be rather accurate. As this song is quite long and has an interesting structure I expected the keygram to do worse.
...
'Master of Puppets', a previously unknown song popularized by 'Stranger Things', is in the key of Em. This would be obvious from the keygram, if you knew that in advance. The keygram is not entirely as clear as the keygram for Stairway. After 200 seconds there is a part where no clear key can be identified from the 'gram and at around 300s the section is estimated to be in Bm, which is a perfect fifth away from Em, so that key change would not be unlikely.

### Limits in Key Estimation
```{r lot}
lotus <-
...
Unlike the previous two examples, it doesn't work for this song at all. It's supposed to be in Em, however from the keygram that is impossible to tell. This means that even for Western music, this method of key estimation is not without flaw.

### Metal with Low Valence
```{r tosir}
to_sirius <-
...
```
'Creep' by Radiohead scores lowest on valence in the Rock half of the corpus and the song scores a lot lower on energy than 'To Sirius' on the previous page. Here the progression is a lot easier to read, as the G, B and C are the pitch classes with the highest magnitude.
...
As 'Creep' has a simple chord progression, its chordogram is clear. The four chords constantly looping are G, B, C and Cm and those are the only four chords in the song.
...
### Metal Song with a Weird Structure
...
The song "Deliverance" by Opeth is part of the Progressive Death Metal subgenre, which explains its peculiar structure. As visible in the cepstrogram the timbre changes at around a 01:10 in, when the vocalist starts singing instead of his usual low growling and more importantly the guitar tone changes from heavily distorted to clean. At roughly 5 minutes into the song, the clean singing starts again, however this part does feature a distorted guitar, so this is not visible in the cepstrogram. The self-similarity matrices highlight the same clean part as an outlier, and both show a highlight at around 9:00, where the instrumental becomes quieter again. If this was a pop-song, this part would be the chorus, although not repeated it does feature the name of the song.
...
### Rock Song with a Weird Structure
...
***
The changes from the softer to the aggressive parts are quite clear from the tempogram, even more than from the SSMs. However the tempogram does not seem to be sure as to what the tempo of the ending is. I had expected it to fall back down to around 80 bpm, so the same as the start of the song. Again there's some visual noise, however it doesn't appear to be at the tempo octaves.
...
***
After some tweaking this KNN algorithm had the best results, this is the one I'll be using in the following experiment. I'll be looking at three genres which are not featured in my main playlist, and see how "Metal" they are according to this classifier.
For this experiment I created three new playlists, a [Glam Metal playlist](https://open.spotify.com/playlist/1DAC3LdsyquzqziE5Ucja1?si=90f62ead04874e5e), a [Grunge playlist](https://open.spotify.com/playlist/1rWZDin7auum63WpMfva5j?si=089b589af6ba4bc2) and finally a [Nu Metal playlist](https://open.spotify.com/playlist/1OVisrN9f3HCJ92DSnRAbd?si=e98523da23dc45bc). As the names suggest two of these genres are subgenres of Metal, however Grunge is a genre in which some artists take heavy influence from Metal. Glam Metal, or Hair Metal as it's sometimes called for the musicians' subtle usage of hairspray, was a popular genre in the '80s. Because of its popularity, the lyrical themes and the overall sound of the music this genre is closer to Hard Rock than to Metal in my opinion. Because I don't listen to this genre at all I predict these songs won't have much of an overlap with the Metal part of my playlist, but because of the small number of features my classifier works with I can't be sure what it will predict.\
Although it I would consider it a subgenre of Alternative Rock, I'd describe Grunge as a mix between Punk Rock and Metal. The Grunge playlist I created consists of five songs that are closer to Metal and five that are closer to Rock, in my opinion at least.\
I chose Nu Metal as the final genre as it is not considered "real Metal" by the type of Metal fan that would curate an online music forum. This Alternative Metal subgenre blends elements of various Metal subgenres, Hip-Hop and numerous other genres including Grunge. Considering the songs in this playlist I think most of them should be considered Metal, but I'm not sure if my KNN classifier will agree.\

### Using the KNN for Classification
```{r knn}
...
My predictions were far off. More songs in the Glam Metal playlist were classed as Metal than I originally thought, so the genre isn't misnamed after all.\
The Grunge playlist was classed as being half Metal, which I hoped for when I picked the songs, however two of the five songs that the KNN calls Metal were from the slower part of the playlist. Especially "Alive" by Pearl Jam had a surprising outcome, as that might be the softest song out of that playlist.\
The Nu-Metal only had three Metal songs in it apparently, but this might be because there are no Nu-Metal songs in my original corpus. Slipknot's "The Heretic Anthem" being classed as Rock was a surprise for me here, especially because the other Slipknot song is considered Metal.\
If I had more songs in my original corpus, and more features with a high correlation with genre these predictions would be a lot more accurate, however this experiment has provided me some important insights into my corpus.

### Conclusion
#### Concluding Thoughts
The difference between Metal music and 'Rock-but-not-Metal' music seems to have something to do with popularity, song length and valence. As most Rock music features electric guitars and drums, the timbre feature has little to do with it. From this portfolio we could conclude the Metal portfolio is overall less popular than the Rock playlist, featuring longer songs with a more negative outlook.

#### Discussion
As expected, the 100 song long playlist was not sufficient to make a clear distinction between Rock and Metal Music. Mainly because of the limited amount of songs, but also because genres don't exist in a vacuum, therefor a song can have Metal elements but be a Rock song, or the other way around. The only real apparent conclusion to be found in this research is that Opeth makes weird music.